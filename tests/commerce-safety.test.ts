...
test('all server quote paths call the same total function', () => {
  for (const path of ['../server/services/createOrder.ts', '../server/api/discounts/quote.post.ts'])
    assert.match(read(path), /calculateOrderTotal\(/, path)
  assert.match(
    read('../server/services/storefrontCheckout.ts'),
    /createDurableOrder\(database, orderInput/,
  )
})
...
test('manual orders are COD-only and begin pending collection', () => {
  ...
  assert.match(service, /paymentMethod: 'cod',\s+paymentStatus: 'pending'/)
...
test('Paymob is absent rather than treated as an unverified payment integration', () => {
  ...
  assert.equal(
    serverFiles.some((path) => /paymob|webhook|callback/.test(path)),
    false,
  )
  const client = files(join(root, 'app'))
    .map((path) => readFileSync(path, 'utf8'))
    .join('\n')
  assert.doesNotMatch(
    client,
    /window\.location|location\.href|paymentStatus\s*=(?!=)|payment_status\s*=(?!=)/,
  )
})
