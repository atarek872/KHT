...
test('standalone catalog preserves original categories, products, prices, images and variants', async () => {
  ...
  assert.deepEqual(
    catalog.categories.map((category) => category.name.en),
    ['T-shirts', 'Tracksuits', 'Trousers'],
  )
  assert.deepEqual(
    catalog.products.map((product) => product.id),
    ['kht-001', 'kht-002', 'kht-003'],
  )
  assert.deepEqual(
    catalog.products.map((product) => product.price),
    [890, 2390, 1290],
  )
  assert.deepEqual(
    catalog.products.map((product) => product.image),
    ['/images/tee.png', '/images/tracksuit.png', '/images/pants.png'],
  )
  assert.deepEqual(
    catalog.products[0]?.sizes.map((size) => size.name),
    ['S', 'M', 'L', 'XL', 'XXL'],
  )
})
...
test('cart remains cookie-persisted and tracking is non-blocking without D1', () => {
  ...
  assert.match(store, /void next[\s\S]{0,30}\.catch/)
  assert.match(store, /syncError\.value/)
...
test('standalone checkout retains server-side shipping and demo boundaries', async () => {
  assert.deepEqual(
    demoShippingZones.map((zone) => zone.rate),
    [60, 70, 90],
  )
  ...
  const sampleDetails = page.slice(
    page.indexOf('function sampleDetails'),
    page.indexOf('async function submit'),
  )
...
test('storefront has no shared account route and no Paymob authority or secrets', () => {
  ...
  const appSources = [
    read('../app/pages/checkout.vue'),
    read('../app/pages/order-confirmation/[reference].vue'),
  ].join('\n')
