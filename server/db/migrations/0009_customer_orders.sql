...
END;
CREATE TRIGGER restock_closed_order AFTER UPDATE OF fulfillment_status ON orders
WHEN NEW.fulfillment_status!=OLD.fulfillment_status AND NEW.fulfillment_status IN ('cancelled','returned') BEGIN
 UPDATE inventory_variants SET stock=stock+COALESCE((SELECT SUM(quantity) FROM order_items WHERE order_id=NEW.id AND variant_id=inventory_variants.id),0)
