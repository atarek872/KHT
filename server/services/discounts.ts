...
type DiscountRow = Omit<
  Discount,
  'active' | 'loginRequired' | 'oncePerCustomer' | 'firstOrderOnly'
> & {
  active: number
...
const mapDiscount = (row: DiscountRow): Discount => ({
  ...row,
...
export function validateDiscount(input: DiscountInput) {
  if (!input.code?.trim().match(/^[A-Z0-9][A-Z0-9_-]{2,31}$/))
  ...
  if (!Number.isInteger(input.value) || input.value <= 0)
    throw new Error('Discount value must be a positive whole number.')
  if (input.type === 'percentage' && input.value > 100)
    throw new Error('Percentage discounts cannot exceed 100%.')
  for (const [label, value] of [
    ['Minimum order', input.minimumOrder],
    ['Maximum discount', input.maximumDiscount],
    ['Usage limit', input.usageLimit],
  ] as const) {
    if (value != null && (!Number.isInteger(value) || value <= 0))
      throw new Error(`${label} must be a positive whole number.`)
  }
  if (input.type === 'fixed' && input.maximumDiscount != null)
    throw new Error('Maximum discount applies only to percentage coupons.')
  if (input.validFrom && Number.isNaN(new Date(input.validFrom).getTime()))
    throw new Error('Valid from is not a valid date.')
  if (input.validUntil && Number.isNaN(new Date(input.validUntil).getTime()))
    throw new Error('Valid until is not a valid date.')
  ...
  if ((input.oncePerCustomer || input.firstOrderOnly) && !input.loginRequired) {
    throw new Error('Per-customer and first-order limits require signed-in customers.')
  }
}
...
export function calculateDiscount(
  ...
  if (discount.usageLimit != null && discount.currentUsage >= discount.usageLimit)
    throw new Error('This coupon has reached its usage limit.')
  if (discount.minimumOrder != null && subtotal < discount.minimumOrder)
    throw new Error(`This coupon requires a minimum order of ${discount.minimumOrder} EGP.`)
  let amount =
    discount.type === 'percentage' ? Math.floor((subtotal * discount.value) / 100) : discount.value
...
export async function listDiscounts(database: D1Database) {
  const result = await database
    ...
    login_required AS loginRequired, once_per_customer AS oncePerCustomer,
    first_order_only AS firstOrderOnly,
    updated_at AS updatedAt FROM discounts ORDER BY updated_at DESC`,
    )
    .all<DiscountRow>()
...
export async function getDiscount(database: D1Database, id: string) {
  const row = await database
    ...
    login_required AS loginRequired, once_per_customer AS oncePerCustomer,
    first_order_only AS firstOrderOnly,
    updated_at AS updatedAt FROM discounts WHERE id = ?`,
    )
    .bind(id)
    .first<DiscountRow>()
...
export async function findDiscountByCode(database: D1Database, code?: string) {
  ...
  const row = await database
    ...
    login_required AS loginRequired, once_per_customer AS oncePerCustomer,
    first_order_only AS firstOrderOnly,
    updated_at AS updatedAt FROM discounts WHERE code = ?`,
    )
...
export async function saveDiscount(
  ...
  await database
    ...
    (id, code, type, value, minimum_order, maximum_discount, usage_limit, current_usage,
     valid_from, valid_until, active, login_required, once_per_customer, first_order_only, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    login_required=excluded.login_required, once_per_customer=excluded.once_per_customer,
    first_order_only=excluded.first_order_only, updated_at=excluded.updated_at`,
    )
    .bind(
      id,
      normalized.code,
      ...
      normalized.loginRequired ? 1 : 0,
      normalized.oncePerCustomer ? 1 : 0,
      normalized.firstOrderOnly ? 1 : 0,
      updatedAt,
    )
    .run()
