...
export interface StorefrontOrderContext {
  userId: string | null
  cartId: string
  cartVersion: number
...
export async function createOrder(
  ...
  if (context && context.expectedTotal !== priced.total)
  ...
  const existingCustomer = input.customer.id
    ...
    : await database
        ...
        .bind(input.customer.phone.trim())
        .first<{ id: string }>()
  ...
  const customerStatement =
    existingCustomer && context
      ...
      : existingCustomer
        ? database
            .prepare(
              `UPDATE customers SET name = ?, phone = ?, email = ?, address = ?, governorate = ?, city = ? WHERE id = ?`,
            )
            .bind(
              input.customer.name.trim(),
              input.customer.phone.trim(),
        ...
        : database
            .prepare(
              `INSERT INTO customers (id, name, phone, email, address, governorate, city) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            )
            .bind(
              customerId,
              input.customer.name.trim(),
  ...
  const statements = [
    ...
    database
      .prepare(
        `INSERT INTO orders (id, number, idempotency_key, customer_id, subtotal,
