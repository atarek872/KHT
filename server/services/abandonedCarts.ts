...
async function configuredAbandonmentMinutes(database: D1Database) {
  const row = await database
    .prepare("SELECT value FROM commerce_settings WHERE key='abandonment_minutes'")
    .first<{ value: string }>()
  const minutes = Number(row?.value)
  return Number.isInteger(minutes) && minutes >= 1 && minutes <= 525600
    ? minutes
    : abandonmentMinutes
}
...
export async function listAbandonedCarts(database: D1Database) {
  const abandonmentMinutes = await configuredAbandonmentMinutes(database)
  const result = await database
    ...
    WHERE state = 'active' AND items_count > 0 AND datetime(last_activity) <= datetime('now', '-${abandonmentMinutes} minutes')
...
export async function getAbandonedCart(
  ...
  const cart = await database
    .prepare(
      `SELECT id, customer_name AS customerName, phone, email,
    subtotal, items_count AS itemsCount, last_activity AS lastActivity, created_at AS createdAt,
    ${recoveryExpression} AS recoveryState FROM abandoned_carts WHERE id = ?`,
    )
    .bind(id)
    .first<CartRow>()
  if (!cart) return null
...
export async function getAbandonedCartMetrics(database: D1Database, since?: string) {
  ...
  const row = await database
    .prepare(
      `SELECT
    COUNT(CASE WHEN state = 'active' AND items_count > 0 AND datetime(last_activity) <= datetime('now', '-${abandonmentMinutes} minutes') THEN 1 END) AS abandonedCount,
