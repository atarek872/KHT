...
const rule = (discount: Discount) =>
  discount.type === 'percentage' ? `${discount.value}% off` : `${money(discount.value)} off`
const date = (value?: string | null) =>
  value
    ? new Intl.DateTimeFormat('en-EG', { dateStyle: 'medium' }).format(new Date(value))
    : 'No limit'
async function toggle(discount: Discount) {
  if (saving.value) return
  saving.value = discount.id
  message.value = ''
  const body: DiscountInput = {
    ...
    loginRequired: discount.loginRequired,
    oncePerCustomer: discount.oncePerCustomer,
    firstOrderOnly: discount.firstOrderOnly,
  }
  try {
    await $fetch(`/api/admin/discounts/${discount.id}`, { method: 'PATCH', body })
    await refresh()
    messageError.value = false
    message.value = `Coupon ${body.active ? 'enabled' : 'disabled'}.`
  } catch {
    messageError.value = true
    message.value = 'The coupon status could not be updated.'
  } finally {
    saving.value = ''
  }
}
...
<template>
  <div class="admin-shell__page admin-discounts-page">
    <AdminPageHeader
      eyebrow="KHT / Commerce"
      title="Discounts"
