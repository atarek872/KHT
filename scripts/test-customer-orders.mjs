...
const insert = db.prepare(
  `INSERT INTO orders(id,number,public_reference,idempotency_key,customer_id,user_id,cart_id,cart_version,subtotal,shipping,total,payment_method,source,shipping_governorate) VALUES(?,?,?,?,'c',?,'cart',?,890,60,950,'cod','website','Cairo')`,
)
assert.throws(() => insert.run('bad', 'bad', 'bad-ref', 'bad', 'v', 1), /CART_CHANGED/)
assert.throws(() => insert.run('stale', 'stale', 'stale-ref', 'stale', 'u', 0), /CART_CHANGED/)
db.exec('BEGIN')
insert.run('rollback', 'rollback', 'rollback-ref', 'rollback', 'u', 1)
assert.throws(
  () =>
    db.exec(
...
assert.equal(db.prepare("SELECT stock FROM inventory_variants WHERE id='kht-001-s'").get().stock, 7)
