...
// Preserve guest shopping: real COD and demo remain separate paths.
const guest = browser()
const guestOrder = (
...
await guest.call(`/api/orders/${guestOrder.id}`)
await b.call(`/api/orders/${guestOrder.id}`, 'GET', undefined, 404)
await guest.call('/api/checkout', 'POST', {
  items,
  demoAcknowledged: true,
  shippingGovernorate: 'Cairo',
})
console.log(
  'PASS: register, guest merge retry, cart save, stale write, logout/login, restore, address, checkout retry, owned order/history, foreign account isolation, CSRF, password revocation, guest COD and guest demo.',
)
